...
double EraseItems(const std::string& file_path, const std::size_t batch_size,
                  MapType& map) {
  using KeyType = typename MapType::key_type;

  std::vector<std::pair<KeyType, bool>> keys(batch_size);
  ...
  while (true) {
    ...
    for (; num_reads < batch_size; ++num_reads) {
      ...
      ss >> keys[num_reads].first >> keys[num_reads].second;
    }
    if (num_reads == 0) {
      break;
    }
...
void RunBench(const std::size_t num_repeats, const std::size_t batch_size,
  ...
  RunBenchmark(
      ...
      [&]() {
        ...
        return EraseItems(file_path, batch_size, map);
      },
      true, "Erase-STL");
...
int main(int argc, char* argv[]) {
  ...
  std::string erase_file_path = "./erase-dataset.txt";
  DataType data_type = DataType::kInvalid;
  std::string data_store_path = "";
  ParseOptions(argc, argv, num_repeats, batch_size, erase_file_path, data_type,
               data_store_path);

  if (data_type == DataType::kInt64) {
    RunBench<uint64_t>(num_repeats, batch_size, erase_file_path);
  } else if (data_type == DataType::kString) {
    RunBench<std::string>(num_repeats, batch_size, erase_file_path);
  } else {
  ...
  if (!data_store_path.empty()) {
    if (data_type == DataType::kInt64) {
      RunBenchMetall<uint64_t>(num_repeats, batch_size, erase_file_path,
                               data_store_path);
    } else if (data_type == DataType::kString) {
      RunBenchMetall<std::string>(num_repeats, batch_size, erase_file_path,
                                  data_store_path);
    } else {
      std::cerr << "Invalid data type" << std::endl;
      return 1;
